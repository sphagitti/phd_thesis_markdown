...
\hypersetup{breaklinks=true,
            bookmarks=true,
            pdfauthor={Teresa Auerbach},
            pdftitle={Time series based anomaly detection in an event driven environment by means of machne learning},
            colorlinks=true,
            citecolor=blue,
            urlcolor=blue,
            linkcolor=magenta,
...
\begin{titlepage}
    ...
    % \ThisULCornerWallPaper{1.0}{style/univ_logo.eps}
        ...
        \huge
        Time series based anomaly detection in an event driven
        environment by means of machne learning

                \vspace{.5cm}

        \Large
        Zeitseriengestützte Anomaliedetektion in einer
        ereignisgesteuerten Umgebung mittels maschinellem Lernen
...
\chapter{The ARIMA model}\label{sec:intro}}

The ARIMA model consists of three different models combined into one and
can be used to make predictions.

\hypertarget{ar-part}{%
\section{AR part}\label{ar-part}}

Values from previous time stamps are used to predict the value for the
next time stamp. The parameter \textbf{p} determines how many past
values are used. Thus, the formula for the AR model can look like this:

\begin{equation}\protect\hypertarget{eq:my_equation}{}{yt = c+$\phi$1yt−1+$\phi$2yt−2+⋯+$\phi$pyt−p+εt}\label{eq:my_equation}\end{equation}

The component \emph{c} is a constant and can be used to shift the
outcome of the prediction. If not desired, it can be set to 0 and be
left out.

The component \emph{εt} is white noise.
...
\hypertarget{i-part}{%
\section{I part}\label{i-part}}

The differencing operation is performed on the time series in order to
make it stationary. The paramter \textbf{d} indicates how often this
operation is performed.

\hypertarget{ma-part}{%
\section{MA part}\label{ma-part}}

Previous forecast errors are used to predict the next value. The
parameter \textbf{q} determines how many past forecast errors are used.
Thus, the formula for the MA model can look like this:

\begin{equation}\protect\hypertarget{eq:my_equation}{}{yt=c+εt+$\theta$1εt−1+$\theta$2εt−2+⋯+$\theta$qεt−q}\label{eq:my_equation}\end{equation}

As in the AR model, the component \emph{c} is a constant to shift the
outcome and the component \emph{εt} is white noise.

\hypertarget{determing-the-parameters}{%
\section{Determing the parameters}\label{determing-the-parameters}}

\hypertarget{parameter-p}{%
\subsection{Parameter p}\label{parameter-p}}

\hypertarget{parameter-d}{%
\subsection{Parameter d}\label{parameter-d}}

\hypertarget{parameter-q}{%
\subsection{Parameter q}\label{parameter-q}}
